...
public class PlusSample {
    ...
    public static void main(String[] args) {
        try {
            ...
            Webmasters webmastersCommandLine = new Webmasters.Builder(new NetHttpTransport(), new JacksonFactory(), credential).setApplicationName("WebmastersCommandLine").build();


            KeywordPositionChecker pc = new KeywordPositionChecker("www.fedesarda.it",
                    webmastersCommandLine,
                    LocalDate.now().minusDays(6),
                    LocalDate.now(), Locale.ITALY);

            SearchAnalyticsQueryResponse searchAnalyticsQueryResponse = pc.checkKeyword("fede sarda");


            System.out.println(searchAnalyticsQueryResponse);
            System.out.println(
                    "viao"
            );
